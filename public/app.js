...
function goToStep(stepIndex) {
  ...
  // Scroll the active step group into view within the sidebar scroll container
  const activeGroup = document.getElementById(`step-group-${stepIndex}`);
  if (activeGroup) {
    // The .sidebar element owns the overflow-y scroll; scroll active group to its top
    const sidebar = activeGroup.closest('.sidebar');
    if (sidebar) {
      const groupTop = activeGroup.offsetTop - sidebar.offsetTop;
      sidebar.scrollTo({ top: groupTop - 12, behavior: 'smooth' });
    } else {
